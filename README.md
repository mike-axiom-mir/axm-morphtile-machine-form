...
The total cell count must be 2..64. An active axis normally requires a non-zero translation step. From v0.16.0 an active axis may instead use `rot_step` for bounded rotation progression while remaining at one translation coordinate. From v0.17.0 a primitive grid axis may likewise use `size_step` while remaining translation-stationary. From v0.18.0 a definition-instance grid axis may use `scale_step` while remaining translation-stationary. From v0.19.0 a definition-instance grid axis may use `with_step` to progress existing finite numeric definition settings while remaining translation-stationary. An active grid axis that has neither translation nor its own validated rotation/size/scale/setting progression still HOLDS.
...
For definition-instance grids only, `scale_step` is also keyed only by `x`, `y`, or `z`. Each provided axis uses either one finite non-zero scalar delta or exactly three finite vector deltas with at least one changed component. All provided grid axes must use the same scalar/vector form. Scalar progression requires scalar `instance.scale` or omitted unit scale; vector progression requires vector `instance.scale` or omitted unit vector `[1,1,1]`. Scalar/vector coercion and mixed scalar/vector axis progression HOLD instead of being guessed. Every generated scale/component across the complete Cartesian domain must remain finite and strictly positive.

For definition-instance grids only, `with_step` is keyed only by `x`, `y`, or `z`. Each provided axis contains 1..32 finite numeric setting deltas. Every stepped setting must already exist as an own finite numeric base in `instance.with`, each progression axis must change at least one setting, and each provided axis must contain at least two cells. Form does not invent positivity or range semantics for definition settings because their meaning belongs to the referenced external definition; it proves only finite deterministic progression and authored-state distinctness before emission.

Grid rotation, primitive size progression, definition scale progression, and definition setting progression compile through the existing fixed loop variables `gx`, `gy`, and `gz`: each affected target component or setting becomes its exact base plus the sum of the authored per-axis loop-index deltas. Single-cell axes remain omitted from the loop tree. Form proves the complete bounded generated authored-state domain before emission. Non-finite expansion, non-positive generated size/scale, or two grid cells collapsing to the same combined position+rotation+scale/size+setting authored state HOLDS. The combined check catches cross-axis cancellation and permits a stationary translation axis only when that axis owns a bounded progression. This is authored-state distinctness, not a visual uniqueness claim for symmetric geometry, unused definition settings, or external definitions.

Primitive `size_step` and definition `scale_step`/`with_step` are target-specific vocabularies and therefore never apply to the same primitive grid target. Definition `scale_step` and `with_step` may coexist with each other and with `rot_step`; primitive `size_step` may coexist with `rot_step`. The same bounded grid rule is reused inside `intent.compose`; it is intentionally not a second looser grid language. MorphTile already owns nested loop variables and expression evaluation, so these producer rules reuse the runtime substrate rather than materializing copied parts or exposing caller-authored expressions.
...
Repeat progression may change existing normalized definition settings, rotation or scalar/vector scale over the fixed repeat index. Grid progression may change definition rotation, bounded scalar/vector scale, and existing finite numeric definition settings over fixed `gx`/`gy`/`gz` indices.
...
- NOT TESTED: visual quality; arbitrary geometry generation; future MorphTile commits beyond the exact pin; whether arbitrary external definition settings have domain-specific semantic or visual effect.
- HELD: autonomous geometry synthesis, recursive/general nested loop/condition/expression recipe synthesis beyond the fixed rules, caller-authored expressions in bounded settings/transforms, scalar/vector scale coercion, automatic definition discovery, visual proof, and production readiness.
